...
public interface RsaDao extends JpaRepository<RSAKey,Serializable> {
    ...
    @Transactional
    @Modifying
    @Query(value = "insert into rsa_key (id, privateKey, publicKey, versionNumber) values (?1,?2,?3,?4)",nativeQuery = true)
    void saveRSAKey(Integer id, byte[] privateKey, byte[] publicKey, int versionNumber);
